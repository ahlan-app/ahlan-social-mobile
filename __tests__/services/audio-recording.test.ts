 ...
 *
 * target: __tests__/services/audio-recording.test.ts
 *
 * test(audio-recording): audio recording implementation
 */
import { audio_recording } from '../../src/services/audio_recording';

describe('audio-recording', () => {
  test('initialises with sane defaults', () => {
    const instance = audio_recording();
    expect(instance).toBeDefined();
  });

  test('handles the happy path', () => {
    const result = audio_recording();
    expect(result.ok).toBe(true);
  });

  test('rejects invalid input', () => {
    expect(() => audio_recording('invalid')).toThrow();
  });
